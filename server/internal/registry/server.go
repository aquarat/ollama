...
func (s *Local) handlePull(w http.ResponseWriter, r *http.Request) error {
	...
	ctx := ollama.WithTrace(r.Context(), &ollama.Trace{
		Update: func(l *ollama.Layer, n int64, err error) {
			...
			fmt.Println("update", l.Digest, l.MediaType, n, err)
	...
	if err := s.Client.Pull(ctx, p.model()); err != nil {
		enc.Encode(progressUpdateJSON{Status: fmt.Sprintf("error: %s", err.Error())})
		return nil
	}
